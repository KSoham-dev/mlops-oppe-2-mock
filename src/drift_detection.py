...
# Add noise
X_drifted["petal_width"] = X_drifted["petal_width"] + np.random.normal(
    0, 0.3, len(X_drifted)
)
...
for metric in original_metrics:
    ...
    logger.info(
        f"{metric.upper()}: {original:.4f} → {drifted:.4f} " f"(↓ {degradation:.2f}%)"
    )
...
perf_drop = (
    (original_metrics["accuracy"] - drifted_metrics["accuracy"])
    / original_metrics["accuracy"]
    * 100
)
logger.info(f"Performance drop: {perf_drop:.2f}%")
